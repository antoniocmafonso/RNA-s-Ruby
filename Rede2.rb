...
# Adicionar zero padding ao final para garantir 12 previsões
while test_inputs_normalized.size < 12
  test_inputs_normalized << [0.0] * window_size
end

real_values = [62, 65, 63.5, 73.5, 79, 97, 105, 110, 100, 90, 70, 50] # Substitua com valores reais
...
# Previsões finais para dados de treinamento
train_predictions_normalized = x_normalized.map { |input| nn.predict(input) }
train_predictions = denormalize(train_predictions_normalized.flatten, rainfall_data.min, rainfall_data.max)

# Plotar previsões finais vs valores reais para dados de treinamento
Gnuplot.open do |gp|
  Gnuplot::Plot.new(gp) do |plot|
    plot.title "Previsões Finais vs Valores Reais (Dados de Treinamento)"
    plot.xlabel "Mês"
    plot.ylabel "Precipitação"
    plot.data << Gnuplot::DataSet.new(rainfall_data[window_size..-1]) do |ds|
      ds.with = "linespoints"
      ds.title = "Valores Reais"
    end
    plot.data << Gnuplot::DataSet.new(train_predictions) do |ds|
      ds.with = "linespoints"
      ds.title = "Previsões"
    end
  end
...
# Fazer previsões finais para dados de teste
predictions_normalized = test_inputs_normalized.map { |input| nn.predict(input) }
predictions = denormalize(predictions_normalized.flatten, test_data.min, test_data.max)

# Garantir que tenhamos exatamente 12 previsões
predictions = predictions.first(12)
...
# Plotar previsões finais vs valores reais usando dados de teste
Gnuplot.open do |gp|
  Gnuplot::Plot.new(gp) do |plot|
    plot.title "Previsões Finais vs Valores Reais (Dados de Teste)"
    plot.xlabel "Mês"
    plot.ylabel "Precipitação"
